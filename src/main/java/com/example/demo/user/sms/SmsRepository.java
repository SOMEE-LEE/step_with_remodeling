...
public class SmsRepository {
	...
	// SMS 인증 정보를 저장하는 메서드
	public void saveSmsCertification(SmsDto smsDto){
	    int LIMIT_TIME = 3 * 60; // 인증 코드의 유효 시간(초), 3분 설정
	    stringRedisTemplate.opsForValue()
	        .set(PREFIX + smsDto.getToNumber(), smsDto.getCertificationCode(), Duration.ofSeconds(LIMIT_TIME)); // Redis에 키와 값을 설정, 유효 시간도 함께 설정
	}
